...
namespace Tristerino
{
    ...
    {
        public static Slider CreateHitChanceSlider(string identifier, string displayName, HitChance defaultValue,
            Menu menu)
        {
        ...
        {
            return
                Math.Abs(source.X*checkPos.Y + source.Y*destination.X + checkPos.X*destination.Y -
                         (checkPos.Y*destination.X + source.X*destination.Y + source.Y*checkPos.X)) < 5;
        }
    }
}